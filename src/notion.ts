...
async function getDataSourcesFromDatabase(databaseId: string) {
  ...
  const database: any = await (getNotionClient() as any).databases.retrieve({ 
    database_id: extractNotionId(databaseId) 
  });
...
export async function handleWrite(payload: WritePayload) {
  ...
  const rawProps: Record<string, any> = { ...(((payload as any).properties) || {}) };
    ...
    if (blocks.length) {
      await getNotionClient().blocks.children.append({ block_id: pageId, children: blocks as any });
    }
  ...
  if (payload.target === "update") {
    ...
    const blocks = toBlocks((payload as any).content);
    if (blocks.length) {
      await getNotionClient().blocks.children.append({ block_id: (payload as any).page_id!, children: blocks as any });
    }
...
export async function handleQuery(payload: QueryPayload) {
  ...
  if (payload.mode === "search") {
    ...
    let has_more = true;
    do {
      const res: any = await (getNotionClient() as any).search({
  ...
  if (payload.mode === "page_get") {
    const res = await (getNotionClient() as any).pages.retrieve({ page_id: (payload as any).page_id! });
    return { mode: "page_get", page: res };
  }

  if (payload.mode === "database_get") {
    const res = await (getNotionClient() as any).databases.retrieve({ 
      database_id: (payload as any).database_id!
    });
    
    // In Notion API 2025-09-03, properties are in data_sources
    ...
    if (res.data_sources && res.data_sources.length > 0) {
      ...
      // Merge data source properties into the database response for backward compatibility
      res.properties = dataSource.properties || {};
      console.log('🔍 Database schema retrieved:', res.id, 'Properties count:', Object.keys(res.properties).length);
    } else {
      console.log('⚠️  No data sources found for database:', res.id);
      res.properties = {};
    }
  ...
  export async function handleCreateDatabase(payload: CreateDatabasePayload) {
    ...
    if (!parentPageId && (payload as any).workspace) {
      ...
      if (dbId) {
        try {
          const db: any = await (getNotionClient() as any).databases.retrieve({ database_id: dbId });
    ...
    if (!parentPageId && (payload as any).workspace) {
      try {
        const holding: any = await (getNotionClient() as any).pages.create({
    ...
    const res: any = await (getNotionClient() as any).databases.create(createPayload);
  ...
  export async function handleBlockGet({ block_id }: any) {
    ...
    const response = await getNotionClient().blocks.retrieve({
      block_id: block_id,
    });
  ...
  export async function handleBlockChildrenAppend({
    ...
    const response = await getNotionClient().blocks.children.append({
      block_id: block_id,
      children: children,
      after: after,
    });
  ...
  export async function handleBlockUpdate({
    ...
    const response = await getNotionClient().blocks.update({
      block_id: block_id,
      ...updateData,
    });
  ...
  export async function handlePagePropertyGet({
    ...
    const response = await getNotionClient().pages.properties.retrieve({
      page_id: page_id,
